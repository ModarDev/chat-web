...
export default async function UserDashboardPage() {
  const user = await requireRole(["USER", "ADMIN", "SUPERADMIN"]);
    ...
    <main className="min-h-screen bg-slate-100 px-6 py-6">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="rounded-2xl border border-blue-100 bg-white p-8 shadow-md shadow-blue-100/60">
          <p className="mb-2 text-sm font-semibold tracking-[0.16em] text-blue-700">USER DASHBOARD</p>
          <h1 className="mb-3 text-3xl font-semibold text-slate-900">Welcome, {user.name}</h1>
          <p className="text-slate-600">คุณเข้าสู่ระบบในบทบาท User และสามารถเริ่มใช้งานระบบได้จากหน้านี้</p>
        </section>
      </div>
    </main>
  );
